...
class MetalCommandQueue internal constructor(ptr: MemorySegment): Managed(ptr, kwm_macos_h::metal_deref_command_queue) {
    companion object {
        fun create(device: MetalDevice): MetalCommandQueue {
            return MetalCommandQueue(ffiDownCall { kwm_macos_h.metal_create_command_queue(device.pointer) })
        }
    }
...
class MetalView internal constructor(ptr: MemorySegment): Managed(ptr, kwm_macos_h::metal_drop_view) {
    companion object {
        fun create(device: MetalDevice): MetalView {
            return MetalView(ffiDownCall { kwm_macos_h.metal_create_view(device.pointer) })
        }
    }

    fun nextTexture(): MetalTexture {
        return MetalTexture(ffiDownCall { kwm_macos_h.metal_view_next_texture(pointer) })
    }
    ...
    fun size(): PhysicalSize {
        return Arena.ofConfined().use { arena ->
            PhysicalSize.fromNative(ffiDownCall { kwm_macos_h.metal_view_get_texture_size(arena, pointer) })
        }
    }

    var isOpaque: Boolean
        get() = ffiDownCall { kwm_macos_h.metal_view_get_is_opaque(pointer) }
