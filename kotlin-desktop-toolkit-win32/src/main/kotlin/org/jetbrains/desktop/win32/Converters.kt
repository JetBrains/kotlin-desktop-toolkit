...
internal fun LogicalSize.toNative(arena: Arena): MemorySegment =
    NativeLogicalSize.allocate(arena).also { result ->
...
internal fun LogicalPoint.toNative(arena: Arena): MemorySegment =
    NativeLogicalPoint.allocate(arena).also { result ->
...
internal fun PhysicalSize.toNative(arena: Arena): MemorySegment =
    NativePhysicalSize.allocate(arena).also { result ->
...
internal fun PhysicalPoint.toNative(arena: Arena): MemorySegment =
    NativePhysicalPoint.allocate(arena).also { result ->
