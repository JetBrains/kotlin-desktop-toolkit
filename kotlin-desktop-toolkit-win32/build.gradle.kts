...
private val defaultTargetPlatform = Platform(Os.WINDOWS, targetArch(project) ?: hostArch())
private val nativeDir = layout.projectDirectory.dir("../native")
private val rustCrateName = "desktop-windows"
...
val enabledPlatforms = listOf(Platform(Os.WINDOWS, Arch.x86_64), Platform(Os.WINDOWS, Arch.aarch64)).filter { crossCompilation.enabled(it) }
...
val compileWin32DesktopToolkitTaskByTarget = buildMap {
...
compileWin32DesktopToolkitTaskByTarget[RustTarget(defaultTargetPlatform, "dev")]?.let { buildNativeTask ->
    tasks.test {
...
val nativeConsumable = configurations.consumable("windowsNativeParts") {
    attributes {
...
val generateBindingsTask = tasks.register<GenerateJavaBindingsTask>("generateBindings") {
    val buildNativeTask = compileWin32DesktopToolkitTaskByTarget[RustTarget(defaultTargetPlatform, "dev")]
    ...
    packageName = "org.jetbrains.desktop.win32.generated"
...
val generateNativeResources = tasks.register<Sync>("generateResourcesDir") {
    ...
    compileWin32DesktopToolkitTaskByTarget.values.forEach { task ->
...
tasks.processResources {
    compileWin32DesktopToolkitTaskByTarget[RustTarget(defaultTargetPlatform, "dev")]?.let {
        dependsOn(it)
    }
}
