...
class SkottieWindow(app: Application) : SkikoWindowWin32(app) {
    ...
    override fun Canvas.draw(size: PhysicalSize, scale: Float, time: Long) {
        val canvas = this
        // Title-bar height is logical; scale it to device pixels to match the physical `size`,
        // so the content box starts right below the custom title bar instead of a hardcoded offset.
        val margin = 12f
        val top = titleBarHeight * scale
        val contentRect = RRect.makeXYWH(
            margin,
            top,
            size.width.toFloat() - margin * 2,
            size.height.toFloat() - top - margin,
            margin,
        )
        Paint().use { paint ->
            paint.color = 0xFF_FF_FF_FF.toInt()
            paint.mode = PaintMode.FILL
            canvas.drawRRect(contentRect, paint)
        }
        Paint().use { paint ->
...
//        window.initializeDropManager()
    ...
    fun handleEvent(event: Event, windowId: WindowId): EventHandlerResult {
        ...
        return when (event) {
            is Event.KeyDown if event.virtualKey == VirtualKey.N && Keyboard.getKeyState(VirtualKey.Control).isDown -> {
                createWindow()
