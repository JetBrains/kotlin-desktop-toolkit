...
import org.jetbrains.desktop.gtk.FontAntialiasingValue
...
import org.jetbrains.desktop.gtk.XdgDesktopSetting
...
private class EditorState {
    ...
    fun draw(canvas: Canvas, y: Float, scale: Float) {
        ...
        val textLine = textLineCreator.makeTextLine(stringLine.toString(), SkikoCustomTitlebar.CUSTOM_TITLEBAR_HEIGHT * scale)
        ...
        Paint().use { paint ->
            ...
            canvas.drawTextLine(textLineStats, 0f, (SkikoCustomTitlebar.CUSTOM_TITLEBAR_HEIGHT * scale) + textLineStats.height, paint)
            canvas.drawTextLine(textLineFps, 600f, (SkikoCustomTitlebar.CUSTOM_TITLEBAR_HEIGHT * scale) + textLineStats.height, paint)
...
internal data class TextLineCreator(
    ...
    fun makeTextLine(text: String, fontSize: Float): TextLine {
        if (textLine == null || this.cachedText != text || this.cachedFontSize != fontSize) {
            ...
            val font = FontMgr.default.matchFamiliesStyle(arrayOf("sans-serif", "Arial"), FontStyle.BOLD)?.let { typeface ->
...
private class WindowContainer(
    private var customTitlebar: SkikoCustomTitlebar?,
    ...
    fun configure(event: Event.WindowConfigure) {
        ...
        if (shouldUseCustomTitlebar) {
            ...
            val titlebarSize = LogicalSize(width = event.size.width, height = SkikoCustomTitlebar.CUSTOM_TITLEBAR_HEIGHT)
            val titlebar = customTitlebar ?: SkikoCustomTitlebar(
                size = titlebarSize,
                titlebarLayout,
                requestClose,
            ).also {
...
fun main(args: Array<String>) {
    ...
    val app = Application()
    val glProcFunc =
        app.getEglProcFunc() ?: app.initializeGl("/System/Library/Frameworks/OpenGL.framework/Versions/A/Libraries/libGL.dylib")!!
    ApplicationState(app, glProcFunc).use { state ->
        app.runEventLoop(
            ApplicationConfig(
                appId = "org.jetbrains.desktop.gtk.skikoSample1",
