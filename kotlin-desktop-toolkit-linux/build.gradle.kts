...
tasks.test {
    val buildNativeTask = compileLinuxDesktopToolkitTaskByTarget[RustTarget(Platform(Os.LINUX, targetArch(project)), "dev")]!!
...
val compileLinuxDesktopToolkitTaskByTarget = listOf(
//    RustTarget(Platform(Os.LINUX, Arch.x86_64), "dev"),
    RustTarget(Platform(Os.LINUX, Arch.aarch64), "dev"),
//    RustTarget(Platform(Os.LINUX, Arch.x86_64), "release"),
    RustTarget(Platform(Os.LINUX, Arch.aarch64), "release"),
...
val clippyCheckTask = tasks.register<Exec>("clippyCheck") {
    val rustTarget = buildPlatformRustTarget(Platform(Os.LINUX, targetArch(project)))
    workingDir = layout.projectDirectory.dir("../native").getAsFile()
    commandLine(
...
compileLinuxDesktopToolkitTaskByTarget[RustTarget(Platform(Os.LINUX, targetArch(project)), "dev")]?.let { buildNativeTask ->
...
val generateNativeResources = tasks.register<Sync>("generateResourcesDir") {
    ...
    compileLinuxDesktopToolkitTaskByTarget.forEach { (_, task) ->
...
tasks.processResources {
    dependsOn(compileLinuxDesktopToolkitTaskByTarget[RustTarget(Platform(Os.LINUX, targetArch(project)), "dev")])
}
