...
public enum class LogLevel {
    ...
    internal fun toNative(): Int {
        return when (this) {
            Off -> desktop_h.NativeLogLevel_Off()
            Error -> desktop_h.NativeLogLevel_Error()
            Warn -> desktop_h.NativeLogLevel_Warn()
            Info -> desktop_h.NativeLogLevel_Info()
            Debug -> desktop_h.NativeLogLevel_Debug()
            Trace -> desktop_h.NativeLogLevel_Trace()
        }
    }
}
...
internal fun initLogger(logFile: Path, consoleLogLevel: LogLevel, fileLogLevel: LogLevel, appender: AppenderInterface) {
    ffiDownCall {
        ...
        Arena.ofConfined().use { arena ->
            ...
            desktop_h.logger_init(configuration)
        }
    }
}
...
private fun checkExceptions(): List<String> {
    return Arena.ofConfined().use { arena ->
        val exceptionsArray = desktop_h.logger_check_exceptions(arena)
...
internal fun <T> ffiDownCall(body: () -> T): T {
    ...
    if (exceptions.isNotEmpty()) {
        desktop_h.logger_clear_exceptions()
