...
import org.jetbrains.desktop.gtk.generated.NativeQueryDragAndDropTarget
...
public class Application : AutoCloseable {
    ...
    init {
        ffiDownCall {
            appPtr = desktop_gtk_h.application_init(applicationCallbacks())
        }
    }
    ...
    public fun runEventLoop(applicationConfig: ApplicationConfig) {
        ...
        ffiDownCall {
            Arena.ofConfined().use { arena ->
                desktop_gtk_h.application_run_event_loop(appPtr!!, arena.allocateUtf8String(applicationConfig.appId))
            }
        }
    }
    ...
    public fun stopEventLoop() {
        ffiDownCall {
            desktop_gtk_h.application_stop_event_loop(appPtr!!)
        }
    }

    public override fun close() {
        ffiDownCall {
            desktop_gtk_h.application_shutdown(appPtr!!)
        }
    }
    ...
    public fun openURL(url: String, activationToken: String?): RequestId? {
        return ffiDownCall {
            Arena.ofConfined().use { arena ->
                ...
                RequestId.fromNativeResponse(desktop_gtk_h.application_open_url(appPtr!!, nativeUrl, nativeActivationToken))
            }
        }
    }
    ...
    public fun openFileManager(path: String, activationToken: String?): RequestId? {
        return ffiDownCall {
            Arena.ofConfined().use { arena ->
                ...
                val nativeActivationToken = activationToken?.let { arena.allocateUtf8String(it) } ?: MemorySegment.NULL
                RequestId.fromNativeResponse(desktop_gtk_h.application_open_file_manager(appPtr!!, nativePath, nativeActivationToken))
            }
        }
    }
    ...
    private fun applicationCallbacks(): MemorySegment {
        ...
        NativeApplicationCallbacks.query_drag_and_drop_target(
            callbacks,
            NativeQueryDragAndDropTarget.allocate(::onQueryDragAndDropTarget, arena),
        )
    ...
    public fun isEventLoopThread(): Boolean {
        return ffiDownCall {
            desktop_gtk_h.application_is_event_loop_thread(appPtr)
        }
    }
    ...
    public fun runOnEventLoopAsync(f: () -> Unit) {
        ffiDownCall {
            ...
            desktop_gtk_h.application_run_on_event_loop_async(appPtr, runOnEventLoopAsyncFunc)
        }
    }

    public fun allScreens(): AllScreens {
        return Arena.ofConfined().use { arena ->
            val screenInfoArray = ffiDownCall { desktop_gtk_h.screen_list(arena, appPtr!!) }
    ...
    public fun clipboardPaste(serial: Int, supportedMimeTypes: List<String>): Boolean {
        return Arena.ofConfined().use { arena ->
            ffiDownCall {
                desktop_gtk_h.application_clipboard_paste(appPtr, serial, mimeTypesToNative(arena, supportedMimeTypes))
            }
        }
    }
    ...
    public fun clipboardPut(mimeTypes: List<String>) {
        Arena.ofConfined().use { arena ->
            ffiDownCall {
                desktop_gtk_h.application_clipboard_put(appPtr, mimeTypesToNative(arena, mimeTypes))
            }
        }
    }

    public fun clipboardGetAvailableMimeTypes(): List<String> {
        val ffiCsvMimetypes = ffiDownCall { desktop_gtk_h.application_clipboard_get_available_mimetypes(appPtr) }
        return try {
    ...
    public fun primarySelectionPaste(serial: Int, supportedMimeTypes: List<String>): Boolean {
        return Arena.ofConfined().use { arena ->
            ffiDownCall {
                desktop_gtk_h.application_primary_selection_paste(appPtr, serial, mimeTypesToNative(arena, supportedMimeTypes))
            }
        }
    }
    ...
    public fun primarySelectionPut(mimeTypes: List<String>) {
        Arena.ofConfined().use { arena ->
            ffiDownCall {
                desktop_gtk_h.application_primary_selection_put(appPtr, mimeTypesToNative(arena, mimeTypes))
            }
        }
    }

    public fun primarySelectionGetAvailableMimeTypes(): List<String> {
        val ffiCsvMimetypes = ffiDownCall { desktop_gtk_h.application_primary_selection_get_available_mimetypes(appPtr) }
        return try {
            splitCsv(ffiCsvMimetypes.getUtf8String(0))
        } finally {
            ffiDownCall { desktop_gtk_h.string_drop(ffiCsvMimetypes) }
    ...
    public fun requestShowNotification(params: ShowNotificationParams): RequestId? {
        return Arena.ofConfined().use { arena ->
            ffiDownCall {
                ...
                RequestId.fromNativeResponse(desktop_gtk_h.application_request_show_notification(appPtr, title, body, soundFilePath))
            }
        }
    }
    ...
    public fun closeNotification(notificationId: UInt) {
        ffiDownCall {
            desktop_gtk_h.application_close_notification(appPtr, notificationId.toInt())
        }
    }

    public fun requestRedrawDragIcon() {
        ffiDownCall {
            desktop_gtk_h.application_request_redraw_drag_icon(appPtr)
        }
    }
}
