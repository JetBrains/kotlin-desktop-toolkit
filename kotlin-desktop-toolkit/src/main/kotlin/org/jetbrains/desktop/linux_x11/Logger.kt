...
public enum class LogLevel {
    ...
    internal fun toNative(): Int {
        return when (this) {
            Off -> desktop_linux_h.NativeLogLevel_Off()
            Error -> desktop_linux_h.NativeLogLevel_Error()
            Warn -> desktop_linux_h.NativeLogLevel_Warn()
            Info -> desktop_linux_h.NativeLogLevel_Info()
            Debug -> desktop_linux_h.NativeLogLevel_Debug()
            Trace -> desktop_linux_h.NativeLogLevel_Trace()
        }
    }
}
...
private fun checkExceptions(): List<String> {
    return Arena.ofConfined().use { arena ->
        val exceptionsArray = desktop_linux_h.logger_check_exceptions(arena)
...
internal fun <T> ffiDownCall(body: () -> T): T {
    ...
    if (exceptions.isNotEmpty()) {
        desktop_linux_h.logger_clear_exceptions()
