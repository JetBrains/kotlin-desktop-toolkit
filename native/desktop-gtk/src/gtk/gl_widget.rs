...
pub struct GlFuncs {
    ...
    pub TexParameteri: extern "C" fn(target: c_uint, pname: c_uint, param: c_int),
...
impl GlFuncs {
    fn new() -> Self {
        Self {
            ...
            TexParameteri: get_egl_f("glTexParameteri").unwrap(),
    ...
    fn resize_texture(&self, texture_id: c_uint, physical_size: PhysicalSize, use_es: bool) {
        ...
        (self.TexParameteri)(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
        (self.TexParameteri)(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
        (self.TexParameteri)(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        (self.TexParameteri)(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        (self.TexImage2D)(
            GL_TEXTURE_2D,
            0,
            GL_RGBA8,
...
pub struct GlWidgetImpl {
    ...
    pub rendering_mode: OnceCell<RenderingMode>,
    pub on_allocate: OnceCell<OnAllocate>,
    pub on_draw: OnceCell<OnDraw>,
}
...
impl gtk4::subclass::widget::WidgetImpl for GlWidgetImpl {
    fn realize(&self) {
        ...
        let use_es = match self.rendering_mode.get().unwrap() {
            RenderingMode::Auto => -1,
            RenderingMode::Gl => 0,
            RenderingMode::GlEs => 1,
        };
    ...
    fn snapshot(&self, snapshot: &gtk4::Snapshot) {
        ...
        let opengl_draw_data = {
            ...
            let Some(data) = data_borrow.as_mut() else { return };
            let gl = &data.gl;
            ...
            if data.texture_size != physical_size {
                ...
                gl.resize_texture(data.texture_id, physical_size, context.uses_es());
            }
        ...
        let on_draw = self.on_draw.get().unwrap();
        on_draw(opengl_draw_data, physical_size, scale);
...
impl GlWidget {
    pub fn new(
        ...
        let imp = obj.imp();
        imp.on_draw.set(Box::new(on_draw)).ok().unwrap();
        imp.rendering_mode.set(rendering_mode).ok().unwrap();

        if let Some(on_allocate) = on_allocate {
