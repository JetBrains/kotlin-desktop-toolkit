...
impl SoftwareRendering {
    ...
    pub fn new(w: Rc<Box<dyn WinitWindow>>) -> anyhow::Result<Self> {
        let context = Context::new(w.clone()).map_err(|e| anyhow!("Failed to create software rendering context: {}", e.to_string()))?;
        let surface = Surface::new(&context, w).map_err(|e| anyhow!("Failed to create software rendering surface: {}", e.to_string()))?;
        Ok(Self { surface })
    }
        ...
        F: FnOnce(SoftwareDrawData) -> bool,
    {
        ...
        let draw_data = SoftwareDrawData {
            canvas: buffer.as_mut_ptr() as *mut u8,
            stride,
        };
