...
use crate::linux::application_api::{DataSource, FfiTransferDataGetter};
use crate::linux::data_transfer::read_all;
use crate::linux::events::{DataTransferContent, DataTransferEvent, EventHandler};
...
type TransferDataGetter = Box<dyn Fn(DataSource, &str) -> Option<Vec<u8>>>;
...
impl gdk4::subclass::content_provider::ContentProviderImpl for ClipboardContentProviderImpl {
    ...
    fn write_mime_type_future(
        ...
        debug!("ContentProviderImpl::write_mime_type_future");
        if let Some(data) = self.get_data_transfer_data.borrow().as_ref().unwrap()(DataSource::Clipboard, mime_type) {
...
impl ClipboardContentProvider {
    pub fn new(mime_types: &MimeTypes, get_data_transfer_data: FfiTransferDataGetter) -> Self {
        let obj = glib::Object::new::<Self>();
        let imp = obj.imp();
        *imp.formats.borrow_mut() = Some(gdk4::ContentFormats::new(&mime_types.val));
        *imp.get_data_transfer_data.borrow_mut() = Some(Box::new(move |data_source, mime_type| {
...
pub fn clipboard_paste_impl(event_handler: EventHandler, serial: i32, mime_types: &MimeTypes, clipboard: &gdk4::Clipboard) {
    debug!("clipboard_paste_impl start");
    let mime_types = &mime_types.val;

    clipboard.read_async(mime_types, glib::Priority::DEFAULT, gio::Cancellable::NONE, move |res| match res {
        ...
        Err(e) => {
            warn!("application_clipboard_paste: clipboard read_async error: {e}");
            let event = DataTransferEvent {
                serial,
                content: DataTransferContent::null(),
            };
    ...
    debug!("clipboard_paste_impl end");
}
