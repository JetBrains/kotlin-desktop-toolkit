use crate::linux::application_state::get_egl;
use gtk4::prelude::{NativeExt, SurfaceExt};

use crate::linux::application_state::EglInstance;
use crate::linux::events::OpenGlDrawData;
use crate::linux::geometry::{LogicalSize, PhysicalSize};
use anyhow::bail;
...
fn get_egl_f<T>(lib: &EglInstance, name: &str) -> anyhow::Result<T> {
    if let Some(f_raw) = lib.get_proc_address(name) {
...
impl GlFuncs {
    fn new(egl: &EglInstance) -> Self {
        Self {
            GetIntegerv: get_egl_f(egl, "glGetIntegerv").unwrap(),
            BindTexture: get_egl_f(egl, "glBindTexture").unwrap(),
            TexImage2D: get_egl_f(egl, "glTexImage2D").unwrap(),
            BindFramebuffer: get_egl_f(egl, "glBindFramebuffer").unwrap(),
            FramebufferTexture2D: get_egl_f(egl, "glFramebufferTexture2D").unwrap(),
            CheckFramebufferStatus: get_egl_f(egl, "glCheckFramebufferStatus").unwrap(),
            DeleteTextures: get_egl_f(egl, "glDeleteTextures").unwrap(),
            DeleteFramebuffers: get_egl_f(egl, "glDeleteFramebuffers").unwrap(),
        }
    }
...
impl gtk4::subclass::widget::WidgetImpl for GlWidgetImpl {
    fn realize(&self) {
        ...
        let egl = get_egl().expect("Couldn't get EGL");
        ...
        let gl_gen_textures: extern "C" fn(c_int, *mut c_uint) = get_egl_f(egl, "glGenTextures").unwrap();
        let gl_gen_framebuffers: extern "C" fn(c_int, *mut c_uint) = get_egl_f(egl, "glGenFramebuffers").unwrap();
...
impl GlWidget {
    pub fn new(do_draw: DrawFn, on_resize: Box<dyn Fn(LogicalSize)>) -> Self {
