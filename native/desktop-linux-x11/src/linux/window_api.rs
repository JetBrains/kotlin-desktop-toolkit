...
use crate::linux::application_api::{AppPtr, DragAndDropActions, RenderingMode};
use crate::linux::events::{RequestId, WindowDecorationMode, WindowId};
...
pub extern "C" fn window_show_menu(app_ptr: AppPtr, window_id: WindowId) {
    ffi_boundary("window_show_menu", || {
        let app = unsafe { app_ptr.borrow::<Application>() };
        app.window_show_menu(window_id)
    });
}
...
pub extern "C" fn window_text_input_enable(app_ptr: AppPtr, window_id: WindowId, context: TextInputContext) {
    with_window(&app_ptr, window_id, "window_text_input_enable", |w| {
        w.text_input_enable(&context);
        Ok(())
    });
}
