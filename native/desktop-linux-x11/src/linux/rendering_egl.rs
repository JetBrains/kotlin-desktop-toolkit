...
use sdl3_sys::video::{SDL_GLContext, SDL_GL_CreateContext, SDL_GL_DestroyContext, SDL_GL_LoadLibrary, SDL_GL_MakeCurrent, SDL_GL_SwapWindow};
...
impl Drop for EglRendering<'_> {
    fn drop(&mut self) {
        ...
        let context = unsafe { SDL_GL_DestroyContext(self.context) };
...
impl<'a> EglRendering<'a> {
    pub fn new(egl: &'a EglInstance, w: *mut SDL_Window) -> anyhow::Result<Self> {
        info!("Trying to use EGL rendering");
        // unsafe { SDL_GL_LoadLibrary() };

        let context = unsafe { SDL_GL_CreateContext(w) };
        if context.is_null() {
        ...
        F: FnOnce(SoftwareDrawData) -> bool,
    {
        ...
        unsafe { SDL_GL_MakeCurrent(self.w, self.context) };
        ...
        if do_draw(SoftwareDrawData::default()) {
            ...
            //     .unwrap();
            unsafe { SDL_GL_SwapWindow(self.w) };
        }
    }
}
