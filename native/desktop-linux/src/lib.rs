...
use smithay_client_toolkit::{
    ...
    delegate_activation, delegate_compositor, delegate_keyboard, delegate_output, delegate_pointer, delegate_registry, delegate_seat,
...
pub fn test_main() {
    ...
    let mut event_loop: EventLoop<SimpleWindow> = EventLoop::try_new().expect("Failed to initialize the event loop!");
...
impl WindowHandler for SimpleWindow {
    ...
    fn configure(&mut self, conn: &Connection, qh: &QueueHandle<Self>, _window: &Window, configure: WindowConfigure, _serial: u32) {
        println!("Window configured to: {configure:?}");

        self.buffer = None;
        self.width = configure.new_size.0.map_or(256, std::num::NonZero::get);
        self.height = configure.new_size.1.map_or(256, std::num::NonZero::get);
