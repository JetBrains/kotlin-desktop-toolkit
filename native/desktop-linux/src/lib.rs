...
use smithay_client_toolkit::reexports::csd_frame::{DecorationsFrame, FrameAction, FrameClick, ResizeEdge};
...
use smithay_client_toolkit::{
    ...
    seat::{
        ...
        pointer::{CursorIcon, PointerData, PointerEvent, PointerEventKind, PointerHandler, ThemeSpec, ThemedPointer},
    },
    shell::{
        WaylandSurface,
        xdg::{
...
pub fn test_main() {
    ...
    let subcompositor_state =
        SubcompositorState::bind(compositor_state.wl_compositor().clone(), &globals, &qh).expect("wl_subcompositor not available");
    ...
    let pool = SlotPool::new(width.get() as usize * height.get() as usize * 4, &shm_state).expect("Failed to create pool");

    let window_surface = compositor_state.create_surface(&qh);

    let window = xdg_shell_state.create_window(window_surface, WindowDecorations::ServerDefault, &qh);
    ...
    fn configure(&mut self, conn: &Connection, qh: &QueueHandle<Self>, window: &Window, configure: WindowConfigure, _serial: u32) {
        ...
        let (width, height) = if configure.decoration_mode == DecorationMode::Client {
            let window_frame = self.window_frame.get_or_insert_with(|| {
                FallbackFrame::new(&self.window, &self.shm_state, self.subcompositor_state.clone(), qh.clone())
