...
impl EventTapThread {
    ...
    fn new() -> Self {
        ...
        let handle = thread::spawn(move || {
            ...
            let (_, _, run_loop) = Self::prepare_tap_subscription(Arc::as_ptr(&events_data_snd)).unwrap(); // todo handle error
            mark_is_ready.send(Ok(RunLoopWrapper(run_loop))).unwrap();
            CFRunLoop::run();
        });

        let run_loop_wrapper = check_is_ready.recv().expect("Can't fail here").unwrap();

        Self { handle: Some(handle), events_data_rcv, run_loop_wrapper }
    }
