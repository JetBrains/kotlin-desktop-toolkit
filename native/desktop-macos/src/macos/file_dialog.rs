...
fn apply_common_param(panel: &NSSavePanel, common_params: &CommonFileDialogParams) {
    panel.setTitle(copy_to_ns_string_if_not_null(&common_params.title).as_deref());
    panel.setPrompt(copy_to_ns_string_if_not_null(&common_params.prompt).as_deref());
    panel.setMessage(copy_to_ns_string_if_not_null(&common_params.message).as_deref());
    panel.setNameFieldLabel(copy_to_ns_string_if_not_null(&common_params.name_field_label).as_deref());
    if let Some(name_field_string_value) = copy_to_ns_string_if_not_null(&common_params.name_field_string_value).as_deref() {
        panel.setNameFieldStringValue(name_field_string_value);
    }
    let directory_url_str = copy_to_ns_string_if_not_null(&common_params.directory_url);
    let directory_url = directory_url_str.map(|directory_url_str| NSURL::fileURLWithPath(&directory_url_str));
    panel.setDirectoryURL(directory_url.as_deref());

    panel.setCanCreateDirectories(common_params.can_create_directories);
    panel.setCanSelectHiddenExtension(common_params.can_select_hidden_extension);

    panel.setShowsHiddenFiles(common_params.shows_hidden_files);
    panel.setExtensionHidden(common_params.extensions_hidden);
}
...
fn run_and_get_path(panel: &NSSavePanel) -> anyhow::Result<Option<Retained<NSString>>> {
    let success = panel.runModal() == NSModalResponseOK;
