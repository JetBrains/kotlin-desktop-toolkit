...
- **Where**: `drag_drop.rs` — `ComInterfaceRawPtr::new(data_object)?` inside `IDropTarget::DragEnter` and the matching `Drop` path.
...
- **Where**: `screen.rs` declares `pub origin: LogicalPoint`; assigned at `screen.rs` via `LogicalPoint::from_physical(…)`.
...
- **Where**: `global_data.rs` — `pub fn new_file_list(file_names: &Vec<&str>)`.
- **Fix**: take `&[&str]` (clippy `ptr_arg`).

### `screen_info_drop` has no caller
- **Where**: `screen_api.rs`. Kotlin calls `screen_list_drop` (which recursively drops elements via `AutoDropArray`). The single-element drop is exported but never invoked from Kotlin.
...
- **Where**: `pointer.rs` + `Pointer.kt`. Rust `PointerModifiers(u32)` populated via `core::mem::transmute` (`pointer.rs`) without named constants.
...
- **Where**: `cursor.rs` — `panic!("Can't create Unknown cursor")`. Triggered if the integer 0 ever crosses FFI from Kotlin (Kotlin omits `Unknown`, but the discriminant 0 is reachable).
...
- **Where**: `org.jetbrains.desktop.common.Platform.kt`. `KotlinDesktopToolkit.kt` re-implements `isAarch64()` locally.
