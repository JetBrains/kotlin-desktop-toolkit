...
**Ownership.** `HMENU` owned by Win32, allocated on the first `GetSystemMenu(hwnd, FALSE)` call (lazy per [Raymond Chen](https://devblogs.microsoft.com/oldnewthing/20100528-00/?p=13893)), persists until the window is destroyed (we never call `GetSystemMenu(hwnd, TRUE)`). The handle is cached in `Window::system_menu` (`Cell<HMENU>`) after `seed_system_menu` runs; `show_system_menu` reads it directly without further `GetSystemMenu` calls.
...
- **Seeding the input queue before `DoDragDrop`** (`seed_drag_input_queue`). `start_drag_drop` posts one `WM_MOUSEMOVE` to the source window immediately before calling `DoDragDrop` — which is why `drag_drop_start` takes a `WindowPtr` at all. `DoDragDrop` establishes no initial state of its own; it fills the cursor position and key state from each message it retrieves, so it makes no progress until one arrives. Because we are called synchronously from the handler for the message that started the drag, that message has already been dispatched and the queue is empty at exactly that moment. Without the seed the loop spins at ~100% of a core without calling `QueryContinueDrag` for as long as 8 seconds, and on a thread whose only input producer is itself — a test robot driving the gesture from the UI thread — it does not proceed at all. With it, the first callback arrives in single-digit milliseconds.

**Gotchas.**
- **The seed's `wParam` must be the live button state.** `ole32` reads the pressed buttons out of that message: a `wParam` of `0` makes `QueryContinueDrag` report a release, so the drag ends immediately with a spurious drop. `seed_drag_input_queue` reads each button with `GetKeyState` at post time rather than hardcoding `MK_LBUTTON` or inheriting the triggering event's state — a right-button drag breaks under either shortcut. Never synthesise a release: over-reporting a button as still down costs one loop iteration, under-reporting ends the drag. `lParam` carries the position for anything that dispatches the message, but the loop itself reads `MSG::pt`, which the system stamps at post time.
- `KDT_WIN32_DND_NO_SEED=1` skips the seed. It exists so the defect stays reproducible; nothing in production should set it.
