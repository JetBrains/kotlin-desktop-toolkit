...
**Files.** `clipboard.rs` (result types), `clipboard_api.rs` (FFI) + Kotlin `Clipboard.kt` (the `Clipboard` object plus `ClipboardResult` / `ClipboardStatus`).
...
**Key types.** `ClipboardResult<T>` (`Success` / `Failure`) and `ClipboardStatus` (failure classification; `Busy` is the retryable contention case — see the `isBusy` extension). `DataObject` is the live COM pointer wrapper handed back by a successful `get`. The throwing `ClipboardException` is retained only for `DataObject`'s `read*` accessors.
...
- Handle contention: when another process holds the clipboard, OLE returns `CLIPBRD_E_CANT_OPEN`, returned as `ClipboardResult.Failure` with `status == ClipboardStatus.Busy` (see `isBusy`). Retrying with backoff is up to the caller.
