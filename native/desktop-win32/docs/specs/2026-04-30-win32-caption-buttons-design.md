...
| `WM_DPICHANGED` | `window.set_dpi_metrics(new_dpi)` (wparam-derived) refreshes the cache first, then `on_dpi_change(new_scale)` re-rasterises glyphs at the new scale, then `on_resize(client_size, window.max_chrome_y())` re-anchors the strip from the now-fresh cache; `client_size` from a fresh `GetClientRect` after the OS-suggested resize | **expensive** (re-rasterise all glyph surfaces, two commits) |
