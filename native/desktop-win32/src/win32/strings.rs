...
#[allow(clippy::cast_sign_loss)]
pub(crate) fn copy_from_utf8_bytes(s: &[u8]) -> anyhow::Result<windows::core::HSTRING> {
    let len = unsafe { MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, s, None) };
    anyhow::ensure!(len != 0, windows::core::Error::from_thread());
    let mut buf = vec![0u16; len as usize + 1];
    let len = unsafe { MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, s, Some(&mut buf)) };
    anyhow::ensure!(len != 0, windows::core::Error::from_thread());
    Ok(windows::core::HSTRING::from_wide(&buf[..len as usize]))
}
