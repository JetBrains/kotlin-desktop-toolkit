//! `CompositionContext` — caption-button D2D / DirectWrite gateway.
//!
...
fn build_d2d_device_with(driver_type: D3D_DRIVER_TYPE, feature_levels: &[D3D_FEATURE_LEVEL]) -> anyhow::Result<ID2D1Device> {
    let d3d_device = create_d3d_device(driver_type, feature_levels)?;
