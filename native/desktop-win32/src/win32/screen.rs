...
pub(crate) fn screen_to_client(window: &Window, point: PhysicalPoint) -> anyhow::Result<PhysicalPoint> {
    let mut screen_point = POINT {
        x: point.x.0,
        y: point.y.0,
    };
    unsafe { ScreenToClient(window.hwnd(), &raw mut screen_point) }.ok()?;
    Ok(PhysicalPoint::new(screen_point.x, screen_point.y))
}
